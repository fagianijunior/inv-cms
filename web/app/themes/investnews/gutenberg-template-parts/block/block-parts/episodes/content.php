<div class="episodes-content">
    <h2>Episódios</h2>
    <div class="swiper episodes-content">
        <?php $episodes = get_field('eps_selector'); 
        if ( $episodes ): ?>
            <div class="swiper-wrapper">
                <?php foreach ( $episodes as $ep ) : setup_postdata( $ep ); ?>
                    <div class="swiper-slide">
                        <a href="<?php the_permalink($ep->ID); ?>" title="<?php echo get_the_title($ep->ID) ?>">
                            <?php if ( has_post_thumbnail($ep->ID) ) { ?>
                                <?php 
                                    echo get_the_post_thumbnail($ep->ID, array(600,394), array( 'class' => 'reg-img lazy' )); 
                                ?>
                            <?php } else { ?>
                        ...
                        <span>Episódio <?php the_field('numero_do_episodio', $ep->ID) ?></span>
                        <h3>
                            <a href="<?php the_permalink($ep->ID); ?>" title="<?php echo get_the_title($ep->ID) ?>">
                                <?php echo get_the_title($ep->ID) ?>
                            </a>
                        </h3>
                    </div>
